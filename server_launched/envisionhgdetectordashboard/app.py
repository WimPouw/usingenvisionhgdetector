...
app = Dash(
    __name__, 
    requests_pathname_prefix='/envisionhgdetectordashboard/',
    assets_folder=ASSETS_DIR,
    serve_locally=True
)

# Important: Define server variable for WSGI
server = app.server

# Load data at startup
gesture_viz_df = pd.read_csv(os.path.join(ASSETS_DIR, "gesture_visualization.csv"))
kinematic_df = pd.read_csv(os.path.join(ASSETS_DIR, "kinematic_features.csv"))
...
def update_visualization(selected_gesture):
    ...
    video_src = ''
    if selected_gesture:
        video_src = f'/envisionhgdetectordashboard/assets/{selected_gesture}_tracked.mp4'
        #video_path = f'assets/{selected_gesture}_tracked.mp4'
        #video_src = f'/envisionhgdetectordashboard/assets/{selected_gesture}_tracked.mp4'
